...
from .config import data_dirs, original_img_files, PROB_THRESHOLDS
import pandas as pd
...
best_idx = grouped_by_threshold['Dice Score'].idxmax()
...
print(f"\nBest threshold based on Dice Score = {best_threshold}")

# Then save to CSV
best_threshold_df = combined_df[combined_df['Threshold'] == best_threshold].copy()
best_threshold_df.to_csv(
    './OCTA_tortuousity/model_eval/final_csvs/best_threshold_deep_metrics.csv',
    index=False
)
