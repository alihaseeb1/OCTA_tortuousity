...
from .config import original_img_files, THRESHOLD_TIS
...
ti_cache = []
data_dir =  "./OCTA_tortuousity/Dataset/test"
data_dirs = [data_dir]
...
for THRESHOLD_TI in THRESHOLD_TIS:
    ...
    if max_f1 < metrics_df["F1 Score"].mean():
        max_f1 = metrics_df["F1 Score"].mean()
        max_thresh = THRESHOLD_TI
    print(metrics_df["F1 Score"])
    f1_scores_final.append(metrics_df["F1 Score"].mean())
...
print(f"Max F1 is : {max_f1} for Threshold TI of {max_thresh}")
print("final metrics are")
print(THRESHOLD_TIS)
# print(weighted_dice_scores_final)
print(f1_scores_final)